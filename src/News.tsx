...
import { Calendar, Play, Pause, Maximize2, X } from 'lucide-react';
...
const News: React.FC = () => {
    ...
    return (
        <div className={`min-h-screen bg-gradient-to-b  mt-16 from-cream to-white ${isRTL ? 'font-tajawal' : 'font-playfair'}`}>
            {/* Header Section - Enhanced with ScrollObserver */}
            <ScrollObserver
                animation="fade-up"
                threshold={0.2}
                delay={100}
            ...
            >
                <div className="container-custom mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center h-full py-20">
                    ...
                    >
                        <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full inline-flex items-center gap-2 mb-8 border border-gold/20 transform hover:scale-105 transition-all duration-300">
                            ...
                            <span className="text-sm text-charcoal font-medium">
                                {isRTL ? '30 مايو 2025' : 'May 30, 2025'}
                            </span>
                        </div>
                    ...
                    {/* Hero Image - Enhanced with smooth animations */}
                    <ScrollObserver
                        animation="fade-left"
                        threshold={0.3}
                        delay={300}
                    ...
                    >
                        <div className="relative animate-custom-bounce">
                            {/* Background decorative element with gentle hover effect */}
                ...
                {/* Floating decorative elements with gentle animation */}
            ...
            {/* Section 1 - Enhanced with ScrollObserver */}
            <ScrollObserver
                animation="fade-up"
                threshold={0.3}
                delay={50}
            ...
            {/* Remaining sections with enhanced image animations */}
            {/* Section 2 */}
            <section
                ref={setSectionRef(1)}
                className="py-20 opacity-0 transform translate-y-10"
            >
                <div className="container-custom mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                    {/* Image */}
                        ...
                        <div className="relative bg-gradient-to-bl from-charcoal/5 to-charcoal/10 p-8 lg:p-16 rounded-3xl group hover:shadow-lg transition-all duration-500"
                            ...
                            <p className="text-lg text-charcoal/80 leading-relaxed group-hover:text-charcoal/90 transition-colors duration-300">
                                {isRTL
                                    ...
                                    : 'The meeting included representatives from the Saudi Ministry of Culture, international design firm Chatillon, and the projecs technical consultant,egis . This collaboration reflects Hebat East'
                                }
                            </p>
                        </div>
                    </div>
            ...
            <section
                ref={setSectionRef(3)}
                className="py-20 bg-gradient-to-br from-charcoal/5 to-gold/5 opacity-0 transform translate-y-10"
            >
                <div className="container-custom mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                    {/* Video Section */}
                    <div className={`${isRTL ? 'lg:order-2' : 'lg:order-1'} relative`}>
                    ...
                    <div className={`${isRTL ? 'lg:order-1' : 'lg:order-2'}`}>
                        <Card className="p-8 lg:p-16 elegant-shadow border-none bg-white/80 backdrop-blur-sm hover:bg-white/90 hover:shadow-xl transition-all duration-500">
                            <blockquote className="text-2xl lg:text-3xl font-bold text-charcoal leading-relaxed mb-6">
                                {isRTL
                                    ? '"يشرفنا أن نكون جزءًا من هذا المشروع الثقافي المهم عبر تنفيذ أعمال تركيب النجف داخل المتحف الأحمر، بما يعكس التزامنا بأعلى معايير الجودة في المشاريع الوطنية الكبري."'
