...
const WhatsAppButton: React.FC = () => {
  ...
  const whatsappNumber = "00966580076800"; // بدون علامة +
  ...
  // إنشاء الرابط بطرق متعددة للتوافق
  ...
  const handleWhatsAppClick = (e: React.MouseEvent) => {
    ...
    // طباعة الرابط في الكونسول للتحقق
    console.log('WhatsApp URL:', url);

    // استخدام location.href للانتقال المباشر بدون نافذة جديدة
    try {
      // على الهاتف - انتقال مباشر
      if (/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        window.location.href = url;
      } else {
        // على الكمبيوتر - نافذة جديدة
        window.open(url, '_blank', 'noopener,noreferrer');
      }
    } catch (error) {
      console.error('Error opening WhatsApp:', error);
      // في حالة الفشل، جرب نسخ الرابط
      navigator.clipboard?.writeText(url).then(() => {
        alert(isRTL ? 'تم نسخ رابط واتساب' : 'WhatsApp link copied');
      });
    }
  };
